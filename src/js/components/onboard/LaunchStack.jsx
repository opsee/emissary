...
const LaunchStack = React.createClass({
  ...
  renderCLI(){
    ...
    return (
      <div>
        <p>To install the role stack using the command line, make sure you have <a target="_blank" href="https://aws.amazon.com/cli/">the AWS CLI</a> set up, then run this command to add the role stack (<em>when we detect the installation we&rsquo;ll advance you to the next step automatically</em>):</p>
        {!!regionLaunchURL ?
          <Highlight>
            <Padding a={1}>
